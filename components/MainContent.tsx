...
export default function MainContent() {
  ...
  return (
    <div className="text-[#F3F4F6] font-sans antialiased selection:bg-[#2C2E31] selection:text-white bg-transparent">
      ...
      <section ref={capabilitiesRef} className="min-h-screen py-20 md:py-40 px-6 md:px-20 border-t border-zinc-800/60 bg-transparent">
        <div className="mb-16 md:mb-28 max-w-4xl">
          ...
          <h2 className="text-2xl md:text-5xl font-extralight tracking-tight leading-tight uppercase text-white">
        ...
        <div className="border-t border-zinc-800/60">
          {services.map((item, index) => (
            <div key={index} className="reveal-row group border-b border-zinc-800/40 py-8 md:py-12 flex flex-col md:flex-row justify-between items-start md:items-center cursor-pointer transition-all duration-700 hover:bg-zinc-900/30 px-2 md:px-4">
              <div className="flex items-start md:items-center gap-6 md:gap-12 w-full md:w-auto">
                <span className="text-[10px] font-mono text-zinc-600 group-hover:text-white transition-colors duration-500 mt-1 md:mt-0">{item.num}</span>
                <h3 className="text-lg md:text-2xl font-light tracking-wide text-zinc-300 group-hover:text-white group-hover:translate-x-3 transition-all duration-500">
                  {item.name}
                </h3>
              </div>
              <p className="text-[11px] md:text-xs text-zinc-500 max-w-md mt-3 md:mt-0 font-light tracking-wide leading-relaxed group-hover:text-zinc-300 transition-colors duration-500 md:text-right pl-10 md:pl-0">
                {item.desc}
              </p>
            </div>
          ))}
      ...
      <section className="min-h-screen py-20 md:py-40 px-6 md:px-20 border-t border-zinc-800/60 bg-transparent">
        <div className="mb-12 md:mb-16">
          <span className="text-[10px] font-mono tracking-[0.5em] text-zinc-400 block mb-2 uppercase">// CASE STUDY IN FOCUS</span>
        </div>
        
        <div className="group border-t border-b border-zinc-800/60 py-16 md:py-24 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative overflow-hidden">
          
          <div className="lg:col-span-7 space-y-8">
            <div>
              <span className="text-[10px] font-mono text-zinc-500 block mb-3">REF. 01/26</span>
              <h3 className="text-4xl md:text-8xl font-light tracking-tighter uppercase text-zinc-400 group-hover:text-white transition-colors duration-500">
            ...
            <div className="inline-block border border-zinc-700 rounded-full px-6 py-3 md:px-8 md:py-4 text-[10px] tracking-[0.3em] font-mono uppercase text-zinc-400 hover:text-white hover:bg-zinc-800 hover:border-zinc-600 transition-all duration-500 cursor-pointer active:scale-95">
          ...
          <div className="lg:col-span-5 w-full">
            ...
            {/* Mobile View: FIX MELAR BUG */}
            {/* Mobile View: COMPATIBLE OPTIMIZED SIZE */}
            <div className="block md:hidden w-full mt-6">
              <div className="flex items-center gap-4 overflow-x-auto snap-x snap-mandatory scrollbar-none px-6 scroll-px-6 pb-4">
                {[1, 2, 3, 4, 5].map((num) => (
                  <div 
                    key={num} 
                    className="relative w-[80vw] h-[80vw] max-w-[340px] max-h-[340px] snap-center shrink-0 border border-zinc-800 bg-zinc-900/20 rounded-sm overflow-hidden shadow-lg"
                  >
                    <img 
                      src={`/pictures/ALILA-${num}.png`} 
                      alt={`Alila Mobile ${num}`} 
                      className="absolute inset-0 w-full h-full object-cover object-center" 
                    />
                  </div>
                ))}
              </div>
              <p className="text-[9px] font-mono tracking-widest text-zinc-500 text-center mt-4">[ SWIPE TO EXPLORE INTERACTIVE → ]</p>
            </div> {/* HAPUS karakter > yang nyasar di sini */}
          </div>
      ...
      {/* 05. PROCESS SECTION */}
      <section ref={methodologyRef} className="min-h-screen py-20 md:py-40 px-6 md:px-20 border-t border-zinc-800/60 bg-transparent">
        <div className="mb-16 md:mb-28">
          <span className="text-[10px] font-mono tracking-[0.5em] text-zinc-400 block mb-4 uppercase">// METHODOLOGY</span>
          <h2 className="text-2xl md:text-5xl font-extralight tracking-tight uppercase text-white">OUR CURATED JOURNEY.</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-5 gap-12 border-t border-zinc-800/60 pt-16">
          {processes.map((proc, idx) => (
            <div key={idx} className="reveal-card space-y-4 md:space-y-6 border-t border-zinc-800/40 pt-6 md:pt-8 hover:border-zinc-500 transition-colors duration-500 group">
      ...
      {/* 06. ABOUT US SECTION */}
      <section className="min-h-screen py-20 md:py-40 px-6 md:px-20 border-t border-zinc-800/60 bg-transparent grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
        <div className="lg:col-span-7 space-y-6">
          <span className="text-[10px] font-mono tracking-[0.5em] text-zinc-400 block uppercase">// ESSENCE</span>
          <h2 className="text-2xl md:text-5xl font-extralight tracking-tight uppercase leading-snug text-zinc-200">
        ...
        <div className="lg:col-span-5 text-zinc-500 text-[11px] md:text-xs font-light tracking-widest leading-loose lg:border-l lg:border-zinc-800/60 lg:pl-16 space-y-4">
          <p>
      ...
      {/* 07. CONTACT SECTION */}
      <section className="min-h-screen py-20 md:py-40 px-6 md:px-20 border-t border-zinc-800/60 flex flex-col justify-between relative bg-transparent">
        <div>
          <span className="text-[10px] font-mono tracking-[0.5em] text-zinc-400 block mb-4 uppercase">// ENGAGEMENT</span>
          <h2 className="text-3xl md:text-7xl font-extralight tracking-tighter uppercase mb-16 md:mb-28 text-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-6xl w-full">
            ...
            {/* WHATSAPP CONTAINER */}
            <div className="border border-zinc-800 p-6 md:p-8 rounded-sm bg-zinc-900/5 backdrop-blur-xs flex flex-col sm:flex-row justify-between items-center gap-6 group hover:border-zinc-700 transition-all duration-300 active:scale-[0.99]">
              <div className="space-y-4 text-center sm:text-left w-full">
                ...
                <a 
                  ...
                  className="text-xl md:text-2xl font-light tracking-wide uppercase text-zinc-400 group-hover:text-white transition-colors duration-300 block active:scale-95 bg-zinc-800/20 sm:bg-transparent py-3 sm:py-0 rounded-full border border-zinc-800 sm:border-none"
                >
                  WHATSAPP
                </a>
                <p className="text-[10px] md:text-xs font-mono text-zinc-500">[ +62 858-0294-9884 ]</p>
              ...
              <div className="w-24 h-24 md:w-32 md:h-32 border border-zinc-800 p-2 bg-zinc-900/40 shrink-0 hidden sm:block">
            ...
            <div className="border border-zinc-800 p-6 md:p-8 rounded-sm bg-zinc-900/5 backdrop-blur-xs flex flex-col sm:flex-row justify-between items-center gap-6 group hover:border-zinc-700 transition-all duration-300 active:scale-[0.99]">
              <div className="space-y-4 text-center sm:text-left w-full">
                ...
                <p className="text-[10px] md:text-xs font-mono text-zinc-500">[ @AISO.CREATIVE ]</p>
                <p className="text-[11px] text-zinc-500 font-light max-w-[200px] leading-relaxed hidden sm:block">
          ...
          <div className="group max-w-6xl border-b border-zinc-800/60 py-8 md:py-10 mt-16 flex flex-col md:flex-row justify-between items-start md:items-center cursor-pointer active:scale-[0.98] transition-transform">
            <a 
              href="mailto:hello@aisocreative.com" 
              className="text-lg md:text-2xl font-light tracking-wider uppercase text-zinc-500 group-hover:text-white transition-colors duration-500"
            >
              GENERAL INQUIRIES
            </a>
            <span className="text-[10px] md:text-[11px] font-mono text-zinc-500 group-hover:text-white transition-colors duration-500 mt-2 md:mt-0">[ HELLO@AISOCREATIVE.COM ]</span>
          </div>

        </div>

        {/* FOOTER */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 pt-20 md:pt-32 text-zinc-600 font-mono text-[9px] tracking-[0.3em] uppercase">
